    ...
    >
      ...
      {products.some(p => !p.price || p.price <= 0) && (
        <motion.div 
          ...
          className="bg-red-50 border border-red-100 p-6 rounded-[32px] flex items-center justify-between gap-6"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-red-600/70 uppercase tracking-widest">
                {products.filter(p => !p.price || p.price <= 0).length} products are missing prices
              </p>
            </div>
          </div>
          <button 
            onClick={() => setActiveTab('products')}
            className="bg-red-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-red-600/20 hover:bg-red-700 transition-all active:scale-95"
          >
            Fix Now
          </button>
        </motion.div>
